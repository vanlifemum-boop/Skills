# Reel 5 — Beweise richtig sichern

**Cover:** `BEWEISE SICHERN. DATEN SCHÜTZEN.`  
**Länge:** 32 Sekunden (4 Clips à 8 s)  
**Fertige Datei:** `~/Medien/JJJJ-MM-TT-tatort-fake-mami/reel-5.mp4`  
**Kosten:** siehe `clip-bibliothek.md` — dieses Reel benutzt 4 Clips, davon werden einige auch von anderen Reels verwendet und nur einmal erzeugt.

---

## Die Clips

| Zeit | Clip | Bild | Was zu sehen ist |
|---|---|---|---|
| 0:00–0:08 | `C04` | `cover-reel5-beweise.png` | Beweisumschlag, Kamera, Schutzschild |
| 0:08–0:16 | `B03` | `beleg-b3-0906.png` | **Kernbeleg** — „am 09.06 von mir Interna an eine andere Mutter weiter gab" |
| 0:16–0:24 | `B05` | `beleg-b5-gruppe-anonym.png` | Gruppe „Mütter Netzwerk Deutschland", viele Entfernungen — Avatar abgedeckt |
| 0:24–0:32 | `C07` | `kanal-motiv.png` | Kanal-Motiv als Abbinder — **einmal erzeugt, in allen sechs Reels verwendet** |

Prompts, Kosten und Einfrier-Bereiche je Clip stehen in `clip-bibliothek.md`.

---

## Text-Einblendungen

Im Schnittprogramm setzen, nicht vom Modell erzeugen lassen.
Unteres Fünftel freilassen.

| Von | Bis | Text | Farbe |
|---|---|---|---|
| 0:00 | 0:03 | BEWEISE SICHERN. DATEN SCHÜTZEN. | weiß, groß |
| 0:03 | 0:07 | Wurden Daten weitergegeben? Erst sichern. | weiß |
| 0:07 | 0:11 | Vollständigen Chatverlauf sichern | weiß |
| 0:11 | 0:15 | Datum, Uhrzeit, Absender, Empfänger festhalten | weiß |
| 0:15 | 0:19 | Originale unverändert aufbewahren | weiß |
| 0:19 | 0:23 | Öffentliche Kopien vollständig schwärzen | weiß |
| 0:23 | 0:28 | ORIGINAL SICHERN. KOPIE SCHWÄRZEN. | rot |
| 0:28 | 0:32 | SACHLICH BLEIBEN. | cyan |

---

## Caption

> Dieser Beitrag dient der sachlichen Warnung und dem Schutz betroffener Mütter.
> Keine Beleidigungen, Drohungen oder Kontaktaufnahme zu beteiligten Personen.
> Private Daten werden gelöscht.
